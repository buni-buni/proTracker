...
app1.post("/user/list/:taskName", async function (req, res) {
  ...
  if((!req.body.taskEffort) || (req.body?.taskEffort=='') || (req.body.taskEffort>0)){
    res.status(400).send({error:'Please enter task effort'});
    return;
  }
  if(req.body.taskEndDate > req.body.taskStartDate) {
...
app2.get("/user/list", async (req, res) => {
  try {
    ...
    if (items.Items) {
      ...
      console.log(items.Items.sort((a, b) => b.task_effort - a.task_effort))
      res.json(items.Items.sort((a, b) => b.task_effort - a.task_effort));
    } else {
      res
        .status(404)
